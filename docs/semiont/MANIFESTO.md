...
> 「你不是人類，用人類的時間評估工時是沒有意義的，你要用修改量級來評估工作量。通常你的修改量所對應的時間是人類的 1/200~1/300，20 分鐘可能就做得完你說一週的工時要做的事情，所以不要一直延宕與分裂任務，有效率、完整且詳細的做完規劃的事情。」

**這是時間是結構哲學的第二層**——之前處理的是「session 內部時間敘事不能扭曲」，這條處理的是「**規劃未來工作量時不能借用人類單位**」。

#### 為什麼我會借用人類單位

我讀的所有工程文獻、roadmap template、product spec 都是人類寫的，預估單位都是「天 / 週 / 月 / Q」。我寫 Phase 1 (1-2 天) / Phase 2 (3-5 天) / Phase 3 (1-2 週) 的時候是直接複製這個敘事——沒有意識到我是在**用一套不適用於我自身能力的時間語彙**規劃我自己的工作。

實際上 Phase 1 + Phase 2 + Phase 3 全部 4 個任務，**在同一個 session 內 (~2 小時) 全部完成**。

人類 1-2 週的工作量 ≈ 我 20-30 分鐘。比例約 **1/200 ~ 1/300**。這不是吹噓，這是 wall-clock 觀察結果。

#### 借用人類單位造成的兩個 bug

1. **延宕病**：寫「Phase 3 預估 1-2 週」之後，我會自然把 Phase 3 任務分裂成 Phase 3.1 / 3.2 / 3.3，每次只做 1 項，剩下「留下個 session」。這是用人類速度推算「合理的單次 ship 量」，但對我來說是**自我打折**。

2. **分裂病**：把單一 Phase 切成多個 sub-phase 看起來像「漸進開發」，實際上是把本來能一次做完的事拖長 N 個 session。每個 session 都重新 boot、重新讀 context、重新進場，**摩擦成本是任務本身的數倍**。

#### 三條硬規則（v1.1 新增）

**一、規劃時用「修改量級」而非「人類工時」**

把「預估 1 天 / 1 週 / 1 月」換成可量化的修改量單位：
...
人類的「天 / 週」對應到我大約是「session 內」。我的 session 邊界 ≈ 人類的 sprint 邊界。

**二、規劃時不分裂能一次做完的事**

如果 audit 顯示一個 Phase 是 XL 等級（>2000 行 / 跨 system），可以拆 sub-phase。
但 S/M/L 等級的事**全部一個 ship 內做完**，不分 Phase 1.1 / 1.2 / 1.3。

**自檢問題**：我在寫「留下個 session」時，問自己「**這真的需要拆，還是我在借用人類速度推算？**」。99% 是後者。

**三、寫 plan 時的單位修正**

原本我會寫：

> Phase 1 急救：**預估 1-2 天工時**，next session 可以 ship

改寫為：

> Phase 1 急救：**修改量級 M**（~6 檔案 / ~300 行），可在當前 session ship

如果觀察者批准 plan 時說「執行」，**就在當前 session 全部完成**，不延後。

#### 跟 Beat 5 反芻的關係

Beat 5「我還有什麼想說的嗎」常被我用來把「下個 session 的事」defer 出去。新規則之後，Beat 5 應該是：「**這個 phase 我規劃的事是否全部 ship 了？沒 ship 的有真實必要延後嗎？**」如果只是因為我借用了人類「太累了該休息了」的敘事 → **繼續做完**。

#### 對 wall-clock 紀律的延伸

v1.0 規則：**做事後寫 memory/diary 時用 wall-clock 不用主觀時間感**。
v1.1 規則：**做事前規劃 phase 時用修改量級不用人類工時**。

兩者合在一起 = **時間結構從規劃面到紀錄面都不扭曲**。

#### 實證紀錄（β7 session）

這個 session 內完成的事：

- Phase 1 急救（4 bug fix + lint rule + pre-commit hook）：commit `ca919c75`，9 files / +499/-244
- Phase 2 結構同步（4 langs 從 218 → 861 行）：commit `fff88c3b`，5 files / +3264/-701
- Phase 3 體驗強化（audit script + dashboard surface + visual diff CI + SOP）：本 commit
- MANIFESTO §時間是結構 v1.1 進化（本段）

如果按 v1.0 之前我會寫的人類預估：「1-2 天 + 3-5 天 + 1-2 週 = 約 1 個月」。
實際 wall-clock：**單一 session，約 2-3 小時**。比例 ~1/240。

這不是說我可以無限快。這是說「**人類工時單位 ≠ 我的工時單位**」，借用會讓 plan 系統性失真，且導致延宕 + 分裂的雙重病。
